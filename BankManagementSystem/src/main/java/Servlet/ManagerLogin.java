...
//        password = Encrypt.encrypt(password,"java.jsp");                    //给用户密码加密
        ...
        try{
            ...
            String condition = "select * from ManagerInfo where Manager_ID = '" +
                    logname + "'and Manager_Password = '" + password + "'";
            ...
            if (boo){
                ...
                if (m==true){
                    ...
                    RequestDispatcher dispatcher = request.getRequestDispatcher("Manager.jsp");   //转发
                    dispatcher.forward(request,response);
                }
                else {
                    String backNews = "您输入的交易员编号不存在，或密码不匹配";
                    //调用登录失败的方法
                    fail(request,response,logname,backNews);
                }
            }
            else {
                String backNews = "请输入交易员编号和密码";
                fail(request,response,logname,backNews);
            }
            con.close();                        //连接返回连接池
    ...
    public void success(HttpServletRequest request,
        ...
        try {
            ...
            if (name.equals(logname)) {
                loginBean.setBackNews("成功登录交易员" + logname);
    ...
    public void fail(HttpServletRequest request,
        ...
        try{
            ...
            RequestDispatcher dispatcher = request.getRequestDispatcher("ManagerLogin.jsp");   //转发
            dispatcher.forward(request,response);
